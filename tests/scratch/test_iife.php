...
if (($P->_seq)($__L_r)) { return $__L_r;}}), '-count', (function($__L__) { if ((call_user_func(function() { $__L_or__3040 = (($__L__->step > 0) ? ($__L__->start < $__L__->end) : false); if ($__L_or__3040) { return $__L_or__3040;} else { if (($__L__->step < 0)) { return ($__L__->start > $__L__->end);} else { return false;}} }))) { $__L_diff = ($__L__->end - $__L__->start);
...
$__L_node_to_insert = (\Clojure\Php\equals($__L_level, $NS->BITS) ? $__L_tail_node : (call_user_func(function() { $__L_child = ($Arr->aget)($__L_parent->arr, $__L_subidx); if ($__L_child) { return ($NS->push_tail)(($__L_level - $NS->BITS), $__L_child, $__L_tail_node);} else { return ($NS->new_path)($__L_parent->edit, ($__L_level - $NS->BITS), $__L_tail_node);} })));
...
$__L_new_root = ((($NS->unsigned_bit_shift_right)($__L_this->cnt, $NS->BITS) > (1 << $__L_this->shift)) ? (call_user_func(function() { $__L_new_root = ($NS->__GT_VectorNode)(null, ($Arr->object_array)($NS->WIDTH)); ($Arr->aset)($__L_new_root->arr, 0, $__L_this->root);
...
$__L_s2 = ($P->_seq)($__L_other);
 while(true) { if (($__L_s1 === null)) { return ($__L_s2 === null);} else { if (($__L_s2 === null)) { return false;} else { if (\Clojure\Php\equals(($P->_first)($__L_s1), ($P->_first)($__L_s2))) { $__recur_0 = ($P->_next)($__L_s1); $__recur_1 = ($P->_next)($__L_s2); $__L_s1 = $__recur_0; $__L_s2 = $__recur_1; continue;} else { if (\Clojure\Php\Kw::create('else')) { return false;}}}} break; }
} else { if (\Clojure\Php\Kw::create('else')) { return false;}}}} break; }}), '-hash', (function($__L_this) {  while(true) { if ($__L_this->_hash) { return $__L_this->_hash;} else { $__L_h = (call_user_func(function() { $__L_s = ($P->_seq)($__L_this); $__L_h = 1;  while(true) { if ($__L_s) { $__recur_0 = ($P->_next)($__L_s); $__recur_1 = ((31 * $__L_h) + ($NS->hash)(($P->_first)($__L_s))); $__L_s = $__recur_0; $__L_h = $__recur_1; continue;} else { $__L_h;
}
 break; } return $__L_s; }));
...
$NS->assoc_node = (function($__L_root, $__L_k, $__L_v) { return \Clojure\Php\assoc((call_user_func(function() { $__L_or__3045 = $__L_root; if ($__L_or__3045) { return $__L_or__3045;} else { return \Clojure\Php\hashMap();} })), $__L_k, $__L_v);});
...
$__L_step = (function() use (&$__L_step, &$__L_m, &$__L_re, &$__L_s) { return ($NS->lazy_seq)((call_user_func(function() { $__L_when_let__3052 = ($NS->re_find)($__L_m); if ($__L_when_let__3052) { $__L_match = $__L_when_let__3052;
...
$__L_or__3059 = (call_user_func(function() { $__L_when_let__3060 = ($P->_namespace)($__L_sym); if ($__L_when_let__3060) { $__L_ns_part = $__L_when_let__3060;
...
$__L_sym = \Clojure\Php\nth($__L___dest_9, 0);
$__L_var = \Clojure\Php\nth($__L___dest_9, 1);
if (((call_user_func(function() { $__L_or__3063 = ($__L_only === null); if ($__L_or__3063) { return $__L_or__3063;} else { return \Clojure\Php\contains(($NS->set)($__L_only), $__L_sym);} })) ? (!\Clojure\Php\contains($__L_exclude, $__L_sym)) : false)) { $__L_sym_PRIME_ = \Clojure\Php\get_($__L_rename, $__L_sym, $__L_sym);
...
$__L_base_val = (call_user_func(function() { $__L_or__3080 = \Clojure\Php\get_(($Clojure_Core->deref)($__L_txn->values), $__L_ref); if ($__L_or__3080) { return $__L_or__3080;} else { return $__L_ref->val;} }));
...
try { return ($NS->binding)(\Clojure\Php\vec($NS->_STAR_current_transaction_STAR_, $__L_txn), (call_user_func(function() { $__L_result = call_user_func($__L_f); ($NS->commit_transaction_BANG_)($__L_txn);
...
$NS->throw_null_pointer = (function(...$__L_msg) { throw ($NS->ex_info)((call_user_func(function() { $__L_or__3088 = \Clojure\Php\first($__L_msg); if ($__L_or__3088) { return $__L_or__3088;} else { return 'Null pointer exception';} })), \Clojure\Php\hashMap(\Clojure\Php\Kw::create('type'), \Clojure\Php\Kw::create('null-pointer')));
});
$NS->inc = (function($__L_x) { return ($__L_x + 1);});
$NS->dec = (function($__L_x) { return ($__L_x - 1);});
...
($NS->println)('Test 1: let in expr context');
($NS->println)((1 + (call_user_func(function() { $__L_x = 2; return $__L_x; }))));
($NS->println)('Test 2: do in expr context');
($NS->println)((10 + (call_user_func(function() { $NS->temp = 5;
return $NS->temp; }))));
($NS->println)('Test 3: nested let');
($NS->println)(((call_user_func(function() { $__L_a = 1; return $__L_a; })) + (call_user_func(function() { $__L_b = 2; return $__L_b; }))));
($NS->println)('Test 4: let with multiple bindings');
($NS->println)((2 * (call_user_func(function() { $__L_x = 3; $__L_y = 4; return ($__L_x + $__L_y); }))));
($NS->println)('Test 5: letfn in expr context');
($NS->println)((100 + (call_user_func(function() { $__L_double = (function($__L_n) use (&$__L_double) { return (2 * $__L_n);}); return 5.0; }))));
($NS->println)('Test 6: do with side effects');
$NS->counter = ($NS->atom)(0);
($NS->println)((1 + (call_user_func(function() { ($NS->swap_BANG_)($NS->counter, $NS->inc);
return ($Clojure_Core->deref)($NS->counter); }))));
($NS->println)('Test 7: let with fn call');
($NS->println)(\Clojure\Php\str_('Result: ', (call_user_func(function() { $__L_nums = \Clojure\Php\vec(1, 2, 3); return ($NS->reduce)($NS->_PLUS_, $__L_nums); }))));
($NS->println)('All IIFE tests passed!');
