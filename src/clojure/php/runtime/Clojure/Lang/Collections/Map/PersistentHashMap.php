...
final class PersistentHashMap extends AbstractPersistentMap
{
    ...
    public function put($key, $value): self
    {
        ...
        // Reuse Box to avoid allocation
        $addedLeaf = Box::reusableFalse();
        $hash = $this->hasher->hash($key);
        $newRoot = $this->root ?? IndexedNode::empty($this->hasher, $this->equalizer);
        $newRoot = $newRoot->put(0, $hash, $key, $value, $addedLeaf);

        if ($newRoot === $this->root) {
            return $this;
        }

        return new self($this->hasher, $this->equalizer, $this->meta, $addedLeaf->value === false ? $this->count : $this->count + 1, $newRoot, $this->hasNull, $this->nullValue);
    }
