...
(def
 ...
 (p/-with-meta ^ (cons (quote fn*) decl) (p/-meta ^ &form))))
...
(def
 ...
 :added "1.0"
 :static true}
 seq? (fn ^:static seq? [x] (satisfies? x)))
...
(def
 ...
 :added "1.0"
 :static true}
 char? (fn ^:static char? [x] (t/char? null x)))
...
(def
 ...
 :added "1.0"
 :static true}
 string? (fn ^:static string? [x] (t/string? null x)))
...
(def
 ...
 :added "1.0"
 :static true}
 map? (fn ^:static map? [x] (satisfies? x)))
...
(def
 ...
 :added "1.0"
 :static true}
 vector? (fn ^:static vector? [x] (satisfies? x)))
...
(def
 ...
 arglist (if (= (quote &form) (first arglist)) 
...
(def 
 ...
 (if (if (p/-equiv (quote fn) ifn)
 (if (t/symbol? null iname) false true))
...
(defn to-array
 ...
 can be any Collection. Maps to t/Collection.toArray()."
 {
 :added "1.0"
 :static true}
...
(defn symbol
 ...
 (cond
 (symbol? name) name
 (t/string? null name) (t/symbol name)
 (t/var? null name) (.toSymbol name)
 (t/keyword? null name) (.sym name)
...
(defn +'
 ...
 See also: +"
 {:inline (nary-inline 'addP) :added "1.0"}
 ([] 0)
 ([x] (cast Number x))
...
(defn +
 ...
 {:inline (nary-inline 'add 'unchecked_add) :added "1.2"}
 ([] 0)
 ([x] (cast Number x))
 ([x y] (h/-add (h/host) x y))
 ([x y & more]
...
(defn *'
 ...
 See also: *"
 {:inline (nary-inline 'multiplyP) :added "1.0"}
 ([] 1)
 ([x] (cast Number x))
...
(defn *
 ...
 longs, will throw on overflow. See also: *'"
 {:inline (nary-inline 'multiply 'unchecked_multiply) :added "1.2"}
 ([] 1)
 ([x] (cast Number x))
 ([x y] (h/-multiply (h/host) x y))
 ([x y & more]
...
(defn /
 ...
 {:inline (nary-inline 'divide) :added "1.0"}
 ([x] (/ 1 x))
...
(defn -'
 ...
 See also: -"
 {:inline (nary-inline 'minusP) :added "1.0"}
 ([x] (h/-negate (h/host) x))
...
(defn -
 ...
 longs, will throw on overflow. See also: -'"
 {:inline (nary-inline 'minus 'unchecked_minus) :added "1.2"}
 ([x] (h/-negate (h/host) x))
 ([x y] (h/-subtract (h/host) x y))
 ([x y & more]
 (reduce1 - (- x y) more)))
...
(defn max
 ...
 {:added "1.0" :inline (nary-inline 'max)}
 ([x] x)
...
(defn min
 ...
 {:added "1.0" :inline (nary-inline 'min)}
 ([x] x)
...
(defn abs
 ...
 If a is a double and ##NaN => ##NaN" :added "1.11"}
 [a]
 (abs a))
...
(defn bit-and
 "Bitwise and"
 {:inline (nary-inline 'and) :added "1.0"}
...
(defn bit-or
 "Bitwise or"
 {:inline (nary-inline 'or) :added "1.0"}
...
(defn bit-xor
 ...
 {:inline (nary-inline 'xor) :added "1.0"}
...
(defn bit-and-not
 ...
 {:inline (nary-inline 'andNot) :added "1.0"
 :static true}
...
(defmacro ..
 ...
 (h/-system-properties (h/host) (get \"os.name\"))
...
(defn ^:private check-valid-options
 ...
 (throw
 (ex-info (apply str "Only these options are valid: "
 (first valid-keys)
...
(defmacro defmulti
 ...
 `(let [v# (def ~mm-name)]
 (when-not (and (.hasRoot v#) (t/multifn? null (deref v#)))
 (def ~mm-name
 (t/multifn (clojure.core/unquote (name mm-name)) (clojure.core/unquote dispatch-fn) (clojure.core/unquote default) (clojure.core/unquote hierarchy))))))))
...
(defmacro ^{:private true} assert-args
 ...
 (throw (ex-info (str (first ~'&form) " requires " ~(second pairs) " in " ~'*ns* ":" (:line (meta ~'&form))))))
...
(defn ^:private deref-future
 ([^t/Future fut]
 (.get fut))
 ([^t/Future fut timeout-ms timeout-val]
...
(defn deref
 ...
 {:added "1.0"
 :static true}
 ([ref] (if (satisfies? ref)
 (.deref ^p/IDeref ref)
 (deref-future ref)))
 ([ref timeout-ms timeout-val]
 (if (satisfies? ref)
 (.deref ^p/IBlockingDeref ref timeout-ms timeout-val)
...
(defn line-seq
 ...
 rdr must implement t/BufferedReader."
 {:added "1.0"
 :static true}
 [^t/BufferedReader rdr]
...
(defn bigint
 ...
 [x] (cond
 (t/bigint? null x) x
 (t/biginteger? null x) (t/bigint-from-biginteger x)
...
(defn biginteger
 ...
 :static true}
 [x] (cond
 (t/biginteger? null x) x
 (t/bigint? null x) (.toBigInteger x)
...
(defn bigdec
 ...
 (t/bigint? null x) (.toBigDecimal x)
 (t/biginteger? null x) (BigDecimal. ^BigInteger x)
...
(defn read
 ...
 t/PushbackReader or some derivee. stream defaults to the
 current value of *in*.
...
(defn read-line
 ...
 :static true}
 []
 (if (t/line-numbering-reader? null *in*)
 (.readLine *in*)
 (.readLine ^t/BufferedReader *in*)))
...
(defmacro time
 ...
 {:added "1.0"}
 [expr]
 `(let [start# (h/-nano-time (h/host))
 ret# ~expr]
 (prn (str "Elapsed time: " (/ (double (- (h/-nano-time (h/host)) start#)) 1000000.0) " msecs"))
 ret#))
...
(defn to-array-2d
 ...
 :static true}
 [^t/Collection coll]
 (let [ret (make-array (. Class (forName "[Lt/Object;")) (. coll (size)))]
...
(defn set
 ...
 (with-meta coll nil)
 (if (satisfies? coll)
 (persistent! (.reduce ^p/IReduceInit coll conj! (transient #{})))
...
(defn ns-publics
 ...
 [ns]
 (let [ns (the-ns ns)]
 (filter-key val (fn [ v] (and (t/var? null v)
 (= ns (.ns v))
 (.isPublic v)))
 (ns-map ns))))
...
(defn refer
 ...
 (when (and to-do (not (satisfies? to-do)))
...
(defmacro with-local-vars
 ...
 (t/push-thread-bindings (hash-map (clojure.core/unquote-splicing name-vals-vec)))
 (try
 ~@body
...
(defn array-map
 ...
 (if (odd? (alength ary))
 (throw (ex-info (str "No value supplied for key: " (last keyvals))))
...
(defn destructure [bindings]
 ...
 bk (val (first bes))
 local (if (t/named? null bb) (with-meta (symbol nil (name bb)) (meta bb)) bb)
...
(defmacro fn
 ...
 (throw (ex-info "(str (if (seq sigs) (str " Parameter declaration " (first sigs) " should be a vector ") (str " Parameter declaration missing ")))" {}))))
 psig (fn* [sig]
 ;; Ensure correct type before destructuring sig
 (when (not (seq? sig))
 (throw (ex-info "(str (str " Invalid signature " sig " should be a list "))" {})))
 (let [[params & body] sig
 _ (when (not (vector? params))
 (throw (ex-info "(str (if (seq? (first sigs)) (str " Parameter declaration " params " should be a vector ") (str " Invalid signature " sig " should be a list ")))" {})))
...
(defmacro for
 ...
 err (fn [& msg] (throw (ex-info "(str (apply str msg))" {})))
...
(defmacro with-in-str
 ...
 {:added "1.0"}
 [s & body]
 `(with-open [s# (-> (t/string-reader ~s) clojure.lang.LineNumberingPushbackReader.)]
 (binding [*in* s#]
 ~@body)))
...
(defn re-pattern
 "Returns an instance of t/Pattern, for use, e.g. in
 re-matcher."
 {:tag t/Pattern
 :added "1.0"
 :static true}
 [s] (if (instance? t/Pattern s)
 s
 (. t/Pattern (compile s))))

(defn re-matcher
 "Returns an instance of t/Matcher, for use, e.g. in
 re-find."
 {:tag t/Matcher
 :added "1.0"
 :static true}
...
(defn re-seq
 ...
 using t/Matcher.find(), each such match processed with
 re-groups."
 {:added "1.0"
 :static true}
 [^t/Pattern re s]
...
(defn re-matches
 ...
 t/Matcher.matches(). Uses re-groups to return the
 groups."
 {:added "1.0"
 :static true}
 [^t/Pattern re s]
...
(defn re-find
 ...
 t/Matcher.find(). Uses re-groups to return the
 groups."
 {:added "1.0"
 :static true}
 ([^t/Matcher m]
...
(defn file-seq
 "A tree seq on t/Files"
 {:added "1.0"
 :static true}
 [dir]
 (tree-seq
 (fn [^t/File f] (. f (isDirectory)))
 (fn [^t/File d] (seq (. d (listFiles))))
 dir))
...
(definline booleans
 "Casts to boolean[]"
 {:added "1.1"}
 [xs] `(t/booleans (clojure.core/unquote xs)))

(definline bytes
 "Casts to bytes[]"
 {:added "1.1"}
 [xs] `(t/bytes (clojure.core/unquote xs)))

(definline chars
 "Casts to chars[]"
 {:added "1.1"}
 [xs] `(t/chars (clojure.core/unquote xs)))

(definline shorts
 "Casts to shorts[]"
 {:added "1.1"}
 [xs] `(t/shorts (clojure.core/unquote xs)))

(definline floats
 "Casts to float[]"
 {:added "1.0"}
 [xs] `(t/floats (clojure.core/unquote xs)))

(definline ints
 "Casts to int[]"
 {:added "1.0"}
 [xs] `(t/ints (clojure.core/unquote xs)))

(definline doubles
 "Casts to double[]"
 {:added "1.0"}
 [xs] `(t/doubles (clojure.core/unquote xs)))

(definline longs
 "Casts to long[]"
 {:added "1.0"}
 [xs] `(t/longs (clojure.core/unquote xs)))
...
(defn seque
 ...
 fill (fn [s]
 (when s
 (if (instance? Exception s) ; we failed to t/offer an error earlier
 (log-error q s)
 (try
 (loop [[x & xs :as s] (seq s)]
 (if s
 (if (t/offer q (if (nil? x) NIL x))
 (recur xs)
 s)
 (when-not (t/offer q q) ; q itself is eos sentinel
 ()))) ; empty seq, not nil, so we know to put eos next time
 (catch Exception e
 (log-error q e))))))
 drain (fn drain []
 (lazy-seq
 (let [x (t/take-blocking q)]
...
(defn- is-annotation? [c]
 (and (class? c)
 (t/assignable-from? t/Annotation c)))

(defn- is-runtime-annotation? [ c]
 (boolean 
 (and (is-annotation? c)
 (when-let [^t/Retention r 
 (t/get-annotation c t/Retention)] 
 (= (.value r) t/RetentionPolicy/RUNTIME)))))
...
(defn- add-annotation [^clojure.asm.AnnotationVisitor av name v]
 ...
 :else (throw (ex-info (str "Unsupported annotation value: " v " of class " (class ev))))))
...
(defn derive
 ...
 (assert (or (class? tag) (and (t/named? null tag) (namespace tag))))

 (alter-var-root #'global-hierarchy derive tag parent) nil)
 ([h tag parent]
 (assert (not= tag parent))
 (assert (or (class? tag) (t/named? null tag)))
 (assert (t/named? null parent))
...
(defn resultset-seq
 ...
 the rows in the t/ResultSet rs"
 {:added "1.0"}
 [^t/ResultSet rs]
...
(defn iterator-seq
 "Returns a seq on a t/Iterator. Note that most collections
...
(defn enumeration-seq
 "Returns a seq on a t/Enumeration"
 {:added "1.0"
 :static true}
 [e]
 (t/enumeration-seq e))
...
(defn format
 "Formats a string using java.lang.String.format, see t/Formatter for format
 string syntax"
 {:added "1.0"
 :static true}
 [fmt & args]
...
(defmacro with-loading-context [& body]
 `((fn loading# [] 
 (t/push-thread-bindings {t/current-loader (.getClassLoader (.getClass loading#))})
 (try
 ~@body
 (finally
 (t/pop-thread-bindings))))))
...
(add-doc-and-meta *in*
 "A t/Reader object representing standard input for read operations.

 Defaults to System/in, wrapped in a LineNumberingPushbackReader"
 {:added "1.0"})

(add-doc-and-meta *out*
 "A t/Writer object representing standard output for print operations.

 Defaults to System/out, wrapped in an OutputStreamWriter"
 {:added "1.0"})

(add-doc-and-meta *err*
 "A t/Writer object representing standard error for print operations.
...
(defn uuid?
 "Return true if x is a t/UUID"
 {:added "1.9"}
 [x] (instance? t/UUID x))

(defn random-uuid
 {:doc "Returns a pseudo-randomly generated t/UUID instance (i.e. type 4).

 See: https://docs.oracle.com/javase/8/docs/api/java/util/UUID.html#randomUUID--"
 :added "1.11"}
 ^t/UUID [] (t/UUID/randomUUID))
...
(defn reduce
 ...
 ([f val coll]
 (if (satisfies? coll)
 (.reduce ^p/IReduceInit coll f val)
...
(defn transduce
 ...
 ([xform f init coll]
 (let [f (xform f)
 ret (if (satisfies? coll)
 (.reduce ^p/IReduceInit coll f init)
...
(defn slurp
 ...
 sw (t/string-writer)]
 (with-open [^t/Reader r (apply jio/reader f opts)]
 (jio/copy r sw)
 (t/to-string sw)))))
...
(defn spit
 ...
 {:added "1.2"}
 [f content & options]
 (with-open [^t/Writer w (apply jio/writer f options)]
 (.write w (str content))))
...
(defn future-cancel
 ...
 {:added "1.1"
 :static true}
 [^t/Future f] (.cancel f true))

(defn future-cancelled?
 "Returns true if future f is cancelled"
 {:added "1.1"
 :static true}
 [^t/Future f] (.isCancelled f))
...
(defn- ^{:dynamic true} assert-valid-fdecl
 ...
 #(if (seq? %)
 (first %)
 (throw (ex-info "(str (if (seq? (first fdecl)) (str " Invalid signature \" " % " \" should be a list ") (str " Parameter declaration \" " % " \" should be a vector ")))" {})))
 fdecl)
 bad-args (seq (remove #(vector? %) argdecls))]
 (when bad-args
 (throw (ex-info "(str (str " Parameter declaration \" " (first bad-args) " \" should be a vector "))" {})))))
...
(defn reader-conditional?
 ...
 {:added "1.7"}
 [value]
 (t/reader-conditional? null value))
...
(defn- data-reader-urls []
 (let [cl (h/-context-class-loader (h/host))]
 (concat
...
(defn- load-data-reader-file [mappings ^t/URL url]
 (with-open [rdr (t/line-numbering-reader (t/input-stream-reader (.openStream url) "UTF-8"))]
...
(defn uri?
 "Return true if x is a t/URI"
 {:added "1.9"}
 [x] (instance? t/URI x))

(defonce ^:private tapset (atom #{}))
(defonce ^:private ^java.util.concurrent.ArrayBlockingQueue tapq (t/array-blocking-queue 1024))

(defonce ^:private tap-loop
 (delay
 (doto (t/thread
 #(let [t (t/take-blocking tapq)
...
(defn tap>
 ...
 [x]
 (force tap-loop)
 (t/offer tapq (if (nil? x) ::tap-nil x)))
...
(defn parse-long
 ...
 (throw (ex-info "(str (parsing-err s))" {}))))
...
(defn parse-uuid
 {:doc "Parse a string representing a UUID and return a t/UUID instance,
 or nil if parse fails.
...
(defn NaN?
 {:doc "Returns true if num is NaN, else false" :added "1.11"}

 [^double num]
 (t/nan? num))

(defn infinite?
 {:doc "Returns true if num is negative or positive infinity, else false" :added "1.11"}
 [^double num]
 (t/infinite? num))
