...
(def
 ...
 (if (= nil (t/maybe-special-tag tag))
 (let [c (t/maybe-class tag false)]
 (if c
...
(defmacro defmulti
 ...
 (def ~mm-name
 (t/multifn ~(name mm-name) ~dispatch-fn ~default ~hierarchy)))))))

(defmacro defmethod
 "Creates and installs a new method of multimethod associated with dispatch-value. "
 {:added "1.0"}
 [multifn dispatch-val & fn-tail]
 `(. ~(with-meta multifn {}) addMethod ~dispatch-val (fn ~@fn-tail)))
...
(defn push-thread-bindings
 ...
 :static true} 
 [bindings]
 (t/push-thread-bindings bindings))
...
(defn binding-conveyor-fn
 ...
 :added "1.3"}
 [f]
 (let [frame (t/clone-thread-binding-frame)]
 (fn 
 ([]
 (t/reset-thread-binding-frame frame)
 (f))
 ([x]
 (t/reset-thread-binding-frame frame)
 (f x))
...
(defn set-agent-send-executor!
 ...
 {:added "1.5"}
 [executor]
 (set! t/pooled-executor executor))

(defn set-agent-send-off-executor!
 "Sets the ExecutorService to be used by send-off"
 {:added "1.5"}
 [executor]
 (set! t/solo-executor executor))
...
(defn send
 ...
 :static true}
 [a f & args]
 (apply send-via t/pooled-executor a f args))
...
(defn send-off
 ...
 :static true}
 [a f & args]
 (apply send-via t/solo-executor a f args))
...
(defn deref
 ...
 (if (satisfies? p/IBlockingDeref ref)
...
(defn sequence
 ...
 (t/transformer-iterator xform (t/iterator coll)))
 ()))
 ([xform coll & colls]
 (or (t/chunk-iterator-seq
 (t/transformer-iteratorMulti
 xform
...
(defn read
 ...
 (t/read-clojure stream (boolean eof-error?) eof-value recursive?))
 ([opts stream]
 (t/read-clojure stream opts)))
...
(defn aget
 ...
 {:added "1.0"}
 ([array idx]
 (t/prep-ret (.getComponentType (class array)) (h/-aget (h/host) array idx)))
 ([array idx & idxs]
...
(defn set
 ...
 (if (set? coll)
 (with-meta coll nil)
 (if (satisfies? p/IReduceInit coll)
...
(defmacro with-local-vars
 ...
 (repeat 't/create-dynamic-var))]
 (t/push-thread-bindings (hash-map ~@name-vals-vec))
 (try
 ~@body
 (finally (t/pop-thread-bindings)))))
...
(defn ns-resolve
 ...
 (t/maybe-resolve-in (the-ns ns) sym))))
...
(defn destructure [bindings]
 ...
 (if (seq ~gmapseq) (first ~gmapseq) t/empty-map))
 ~gmap))
 ((fn [ret]
 (if (:as b)
 (conj ret (:as b) gmap)
 ...
 bk (val (first bes))
 local (if (t/named? bb) (with-meta (symbol nil (name bb)) (meta bb)) bb)
...
(defmacro with-in-str
 ...
 {:added "1.0"}
 [s & body]
 `(with-open [s# (-> (java.io.StringReader. ~s) t/line-numbering-reader)]
 (binding [*in* s#]
 ~@body)))
...
(defn mix-collection-hash
 ...
 :static true}
 ^long
 [^long hash-basis ^long count] (t/mix-coll-hash hash-basis count))
...
(defn descendants
 ...
 ([h tag] (if (class? tag)
 (throw (ex-info "Unsupported operation" "Can't get descendants of classes"))
...
(defn derive
 ...
 (assert (or (class? tag) (and (t/named? tag) (namespace tag))))

 (alter-var-root #'global-hierarchy derive tag parent) nil)
 ([h tag parent]
 (assert (not= tag parent))
 (assert (or (class? tag) (t/named? tag)))
 (assert (t/named? parent))
...
(defn resultset-seq
 ...
 keys (map (comp keyword #(.toLowerCase %))
 (map (fn [i] (. rsmeta (getColumnLabel i))) idxs))
 check-keys
 (or (apply distinct? keys)
 (throw (Exception. "ResultSet must have unique column labels")))
 row-struct (apply create-struct keys)
 row-values (fn [] (map (fn [i] (. rs (getObject i))) idxs))
 rows (fn thisfn []
 (when (. rs (next))
...
(defmacro with-loading-context [& body]
 `((fn loading# [] 
 (t/push-thread-bindings {t/current-loader 
...
(defmacro ns
 ...
 ~@(when name-metadata
 `((.resetMeta (t/find-ns '~name) ~name-metadata)))
...
(defn- throw-if
 ...
 (throw (t/compiler-exception
 *file*
 (.deref t/current-line)
 (.deref t/current-column)
 exception)))))
...
(defn intern
 ...
 :static true}
 ([ns name]
 (let [v (t/intern-var (the-ns ns) name)]
 (when (meta name) (.setMeta v (meta name)))
 v))
 ([ns name val]
 (let [v (t/intern-var (the-ns ns) name val)]
 (when (meta name) (.setMeta v (meta name)))
 v)))
...
(defn transduce
 ...
 ([xform f init coll]
 (let [f (xform f)
 ret (if (satisfies? p/IReduceInit coll)
 (.reduce coll f init)
...
(defn future-call 
 ...
 fut (.submit t/solo-executor ^Callable f)]
 (reify 
 p/IDeref 
...
(defn tagged-literal
 ...
 {:added "1.7"}
 [tag form]
 (t/tagged-literal tag form))

(defn reader-conditional?
 "Return true if the value is the data representation of a reader conditional"
 {:added "1.7"}
 [value]
 (t/reader-conditional? value))
...
(defn reader-conditional
 ...
 {:added "1.7"}
 [form splicing?]
 (t/reader-conditional form splicing?))
...
(defn- load-data-reader-file [mappings ^java.net.URL url]
 (with-open [rdr (t/line-numbering-reader
