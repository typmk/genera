...
(def
 ...
 cons (fn* ^:static cons [x seq] (t/cons x seq)))
...
(def
 ^{:arglists '([coll])
   ...
   :added "1.0"
   :static true}
 first (fn ^:static first [coll] (let [s# (p/-seq coll)] (when s# (p/-first s#)))))
...
(def
 ^{:arglists '([coll])
  ...
  argument.  If there are no more items, returns nil."
   :added "1.0"
   :static true}  
 next (fn ^:static next [x] (p/-next x)))
...
(def
 ^{:arglists '([coll])
  ...
  argument."
   :added "1.0"
   :static true}  
 rest (fn ^:static rest [x] (p/-rest x)))
...
(def
 ...
 conj (fn ^:static conj
        ([] [])
        ([coll] coll)
        ([coll x] (p/-conj coll x))
        ([coll x & xs]
         (if xs
           (recur (p/-conj coll x) (first xs) (next xs))
           (p/-conj coll x)))))
...
(def
 ^{:arglists '( [coll])
   ...
   :added "1.0"
   :static true}
 seq (fn ^:static seq  [coll] (p/-seq coll)))
...
(def
 ^{:arglists '([ c x])
   ...
   :added "1.0"}
 instance? (fn instance? [ c x] (. c (isInstance x))))
 ...
 defn (fn defn [&form &env name & fdecl]
        ...
        (let [m (if (string? (first fdecl))
              ...
              m (let [inline (:inline m)
                  ...
                  (if (if (p/-equiv 'fn ifn)
                        (if (t/symbol? iname) false true))
                    ;; inserts the same fn name to the inline fn if it does not have one
                    (assoc m :inline (cons ifn (cons (t/symbol (.concat (.getName  name) "__inliner"))
                                                     (next inline))))
                    m))
...
(defn sorted-map-by
  ...
  {:added "1.0"
   :static true}
  ([comparator & keyvals]
   (t/sorted-map comparator keyvals)))
...
(defn nil?
  ...
  {:tag Boolean
   :added "1.0"
   :static true
   :inline (fn [x] (list '(h/-identical? (h/host)) x nil))}
...
(defn symbol
  ...
  ([name]
     (cond
      (symbol? name) name
      (t/string? name) (t/symbol name)
      (t/var? name) (.toSymbol  name)
      (t/keyword? name) (.sym  name)
      :else (throw (ex-info "no conversion to symbol" {}))))
...
(defn gensym
  ...
  {:added "1.0"
   :static true}
  ([] (gensym "G__"))
  ([prefix-string] (t/symbol (str prefix-string (str (t/next-id))))))
...
(defmacro lazy-seq
  ...
  {:added "1.0"}
  [& body]
  (list 'new 't/LazySeq (list* '^{:once true} fn* [] body)))    
...
(defn ^:static chunk-cons [chunk rest]
  (if (h/-zero? (h/host) (p/-count chunk))
    rest
...
(defmacro delay
  ...
  {:added "1.0"}
  [& body]
    (list 'new 't/Delay (list* `^{:once true} fn* [] body)))
...
(defn zero?
  ...
  {
   :inline (fn [num] `(h/-zero? (h/host) ~num))
   :added "1.0"}
...
(defn nth
  ...
  {:inline (fn  [c i & nf] `(p/-nth ~c ~i ~@nf))
   :inline-arities #{2 3}
   :added "1.0"}
  ([coll index] (p/-nth coll index))
  ([coll index not-found] (p/-nth coll index not-found)))

(defn <
  "Returns non-nil if nums are in monotonically increasing order,
  otherwise false."
  {:inline (fn [x y] `(h/-lt (h/host) ~x ~y))
   :inline-arities #{2}
   :added "1.0"}
  ([x] true)
  ([x y] (h/-lt (h/host) x y))
  ...
  ([op unchecked-op]
     (fn
       ([x] (let [op (if *unchecked-math* unchecked-op op)]
              `((resolve (symbol "clojure.core" (name ~'op))) ~~'x)))
       ([x y] (let [op (if *unchecked-math* unchecked-op op)]
                `((resolve (symbol "clojure.core" (name ~'op))) ~~'x ~~'y)))
       ([x y & more]
          (let [op (if *unchecked-math* unchecked-op op)]
            (reduce1
             (fn [a b] `((resolve (symbol "clojure.core" (name ~'op))) ~~'a ~~'b))
             `((resolve (symbol "clojure.core" (name ~'op))) ~~'x ~~'y) more))))))

(defn ^:private >1? [n] (h/-gt (h/host) n 1))
(defn ^:private >0? [n] (h/-gt (h/host) n 0))
...
(defn <=
  ...
  otherwise false."
  {:inline (fn [x y] `(h/-lte (h/host) ~x ~y))
   :inline-arities #{2}
   :added "1.0"}
  ([x] true)
...
(defn >=
  ...
  otherwise false."
  {:inline (fn [x y] `(h/-gte (h/host) ~x ~y))
   :inline-arities #{2}
   :added "1.0"}
  ([x] true)
...
(defn ==
  ...
  {:inline (fn [x y] `(h/-num-equiv (h/host) ~x ~y))
   :inline-arities #{2}
   :added "1.0"}
  ([x] true)
...
(defn unchecked-remainder-int
  ...
  {
   :added "1.0"}
  [x y] (unchecked-remainder-int x y))

(defn pos?
  "Returns true if num is greater than zero, else false"
  {
   :inline (fn [num] `(h/-pos? (h/host) ~num))
   :added "1.0"}
...
(defn neg?
  ...
  {
   :inline (fn [num] `(h/-neg? (h/host) ~num))
   :added "1.0"}
...
(defn bit-clear
  ...
  {:added "1.0"
   :static true}
  [x n] (h/-bit-clear (h/host) x n))

(defn bit-set
  "Set bit at index n"
  {:added "1.0"
   :static true}
  [x n] (h/-bit-set (h/host) x n))

(defn bit-flip
  "Flip bit at index n"
  {:added "1.0"
   :static true}
  [x n] (h/-bit-flip (h/host) x n))

(defn bit-test
  "Test bit at index n"
  {:added "1.0"
   :static true}
  [x n] (h/-bit-test (h/host) x n))
...
(defn even?
  ...
  {:added "1.0"
   ...
   [n] (if (integer? n)
        ...
        (throw (ex-info "Argument must be an integer: " n {}))))
...
(defn map-entry?
  ...
  {:added "1.8"}
  [x]
  (satisfies? clojure.lang.protocols/IMapEntry x))
...
(defn contains?
  ...
  {:added "1.0"
   :static true}
  [coll key] (p/-contains-key? coll key))

(defn get
  "Returns the value mapped to key, not-found or nil if key not present
  in associative collection, set, string, array, or ILookup instance."
  {:inline (fn  [m k & nf] `(p/-lookup ~m ~k ~@nf))
   :inline-arities #{2 3}
   :added "1.0"}
  ([map key]
   (p/-lookup map key))
  ([map key not-found]
   (p/-lookup map key not-found)))
...
(defn select-keys
  ...
  [map keyseq]
    (loop [ret {} keys (seq keyseq)]
      (if keys
        (let [entry (t/find map (first keys))]
          (recur
           (if entry
             (conj ret entry)
             ret)
...
(defmacro defmulti
  ...
  (let [docstring   (if (string? (first options))
    ...
    (let [options   (apply hash-map options)
      ...
      `(let [v# (def ~mm-name)]
         (when-not (and (.hasRoot v#) (t/multifn? (deref v#)))
           (def ~mm-name
...
(defn add-watch
  ...
  {:added "1.0"
   :static true}
  [ reference key fn] (.addWatch reference key fn))
...
(defn ref
  ...
  ([x & options] 
   (let [r   (setup-reference (ref x) options)
         opts (apply hash-map options)]
...
(defn ^:private deref-future
  ([fut]
   (clojure.concurrent/-deref-future fut))
  ([fut timeout-ms timeout-val]
   (clojure.concurrent/-deref-future-timeout fut timeout-ms timeout-val)))
...
(defn swap-vals!
  ...
  {:added "1.9"}
  ( [2 atom f] (.swapVals atom f))
  ( [2 atom f x] (.swapVals atom f x))
  ( [2 atom f x y] (.swapVals atom f x y))
  ( [2 atom f x y & args] (.swapVals atom f x y args)))
...
(defn compare-and-set!
  ...
  {:added "1.0"
   :static true}
  [ atom oldval newval] (.compareAndSet atom oldval newval))
...
(defn reset-vals!
  ...
  {:added "1.9"}
   [2 atom newval] (.resetVals atom newval))
...
(defn alter-meta!
  ...
  {:added "1.0"
   :static true}
 [ iref f & args] (.alterMeta iref f args))

(defn reset-meta!
  "Atomically resets the metadata for a namespace/var/ref/agent/atom"
  {:added "1.0"
   :static true}
 [ iref metadata-map] (.resetMeta iref metadata-map))
...
(defn sequence
  ...
  ([coll]
     (if (seq? coll) coll
         (or (seq coll) ())))
  ([xform coll]
     (or (t/chunk-iterator-seq
         (clojure.lang.TransformerIterator/create xform (t/iterator coll)))
       ()))
  ([xform coll & colls]
     (or (t/chunk-iterator-seq
         (clojure.lang.TransformerIterator/createMulti
           xform
           (map #(t/iterator %) (cons coll colls))))
       ())))
...
(defn drop
  ...
  ([n coll]
     (if (instance? clojure.lang.IDrop coll)
       (or
        (if (pos? n)
          (.drop  coll (if (int? n) n (Math/ceil n)))
          (seq coll))
        ())
       (let [step (fn [n coll]
                    (let [s (seq coll)]
...
(defn line-seq
  ...
  rdr must satisfy clojure.io/ILineReader."
  {:added "1.0"
   :static true}
  [rdr]
  (when-let [line (clojure.io/-read-line rdr)]
    (cons line (lazy-seq (line-seq rdr)))))

(defn comparator
  "Returns a comparator function based upon pred. The comparator returns
  -1, 0, or 1 depending on the predicate."
  {:added "1.0"
   :static true}
  [pred]
    (fn [x y]
...
(defn sort
  ...
  supplied, uses compare.  Comparator should be a function taking two
  arguments and returning negative, zero, or positive.  Guaranteed to
  be stable: equal elements will not be reordered.  If coll is an array,
  it will be modified.  To avoid this, sort a copy of the array."
  {:added "1.0"
   :static true}
  ([coll]
   (sort compare coll))
...
(defn await
  ...
  (io! "await in transaction"
    ...
    (let [latch (clojure.concurrent/countdown-latch (count agents))
          count-down (fn [agent] (clojure.concurrent/-countdown latch) agent)]
      (doseq [agent agents]
        (send agent count-down))
      (clojure.concurrent/-await-latch latch))))

(defn ^:static await1 [ a]
  ...
  [timeout-ms & agents]
    (io! "await-for in transaction"
     ...
     (let [latch (clojure.concurrent/countdown-latch (count agents))
       ...
       (doseq [agent agents]
           (send agent count-down))
       (clojure.concurrent/-await-latch-timeout latch timeout-ms)))))
...
(defn into-array
  ...
  {:added "1.0"
   :static true}
  ([aseq]
     (t/seq-to-array (seq aseq)))
  ([type aseq]
     (t/seq-to-array type (seq aseq))))
...
(defn unchecked-byte
  ...
  {:inline (fn  [x] `(unchecked-byte ~x))
   :added "1.3"}
  [ x] (unchecked-byte x))

(defn unchecked-short
  "Coerce to short. Subject to rounding or truncation."
  {:inline (fn  [x] `(unchecked-short ~x))
   :added "1.3"}
  [ x] (unchecked-short x))

(defn unchecked-char
  "Coerce to char. Subject to rounding or truncation."
  {:inline (fn  [x] `(unchecked-char ~x))
   :added "1.3"}
  [x] (unchecked-char x))

(defn unchecked-int
  "Coerce to int. Subject to rounding or truncation."
  {:inline (fn  [x] `(unchecked-int ~x))
   :added "1.3"}
  [ x] (unchecked-int x))
...
(defn bigdec
  ...
  [x] (cond
       ...
       (t/bigint? x) (.toBigDecimal  x)
       (t/biginteger? x) (BigDecimal. ^BigInteger x)
...
(defn to-array-2d
  ...
  [^java.util.Collection coll]
    (let [ret (make-array (h/-array-class (h/host)) (. coll (size)))]
...
(defn load-string
  ...
  {:added "1.0"
   :static true}
  [s]
  (let [rdr (-> (clojure.io/string-reader s)
...
(defn set
  ...
  (if (set? coll)
    ...
    (if (instance? clojure.lang.IReduceInit coll)
      (persistent! (.reduce  coll conj! (transient #{})))
...
(defn ns-publics
  ...
  [ns]
  (let [ns (the-ns ns)]
    (filter-key val (fn [ v] (and (t/var? v)
                                 (= ns (.ns v))
                                 (.isPublic v)))
                (ns-map ns))))
...
(defn refer
  ...
  [ns-sym & filters]
    (let [ns (or (find-ns ns-sym) (throw (new Exception (str "No namespace: " ns-sym))))
      ...
      (when (and to-do (not (satisfies? p/ISequential to-do)))
        (throw (new Exception ":only/:refer value must be a sequential collection of symbols")))
      (doseq [sym to-do]
        (when-not (exclude sym)
          (let [v (nspublics sym)]
            (when-not v
              (throw (ex-info (if (get (ns-interns ns) sym)
                                (str sym " is not public")
                                (str sym " does not exist"))
                              {:sym sym :ns ns :type :illegal-access})))
...
(defn array-map
  ...
  ([& keyvals]
     (let [ary (to-array keyvals)]
       (if (odd? (alength ary))
         (throw (ex-info "No value supplied for key: " (last keyvals {})))
         (t/array-mapAsIfByAssoc ary)))))
...
(defn seq-to-map-for-destructuring
  ...
  [s]
  (if (next s)
    (t/array-mapAsIfByAssoc (to-array s))
             ...
             (let [pvec
                   ...
                   (fn [bvec b v]
                     (let [gmap (gensym "map__")
                           gmapseq (with-meta gmap {:tag 'p/ISeq})
                           defaults (:or b)]
                       (loop [ret (-> bvec (conj gmap) (conj v)
                                      (conj gmap) (conj `(if (seq? ~gmap)
                                                           (if (next ~gmapseq)
                                                             (t/array-mapAsIfByAssoc (to-array ~gmapseq))
  ...
  [& sigs]
    (let [name (if (symbol? (first sigs)) (first sigs) nil)
          ...
          psig (fn* [sig]
                 ...
                 (when (not (seq? sig))
                   (throw (ex-info "Invalid signature " sig
                                 " should be a list" {})))
                 (let [[params & body] sig
                       _ (when (not (vector? params))
                           (throw (ex-info (if (seq? (first sigs {}))
...
(defmacro with-out-str
  ...
  {:added "1.0"}
  [& body]
  `(let [s# (clojure.io/string-writer)]
     (binding [*out* s#]
       ~@body
       (clojure.io/string-writer-value s#))))
...
(defmacro with-in-str
  ...
  {:added "1.0"}
  [s & body]
  `(with-open [s# (-> (clojure.io/string-reader ~s) clojure.lang.LineNumberingPushbackReader.)]
     (binding [*in* s#]
       ~@body)))
...
(defn re-pattern
  "Returns a compiled regex pattern, for use, e.g. in re-matcher."
  {:added "1.0"
   :static true}
  [s] (if (clojure.regex/pattern? s)
        s
        (clojure.regex/pattern s)))

(defn re-matcher
  "Returns a regex matcher, for use, e.g. in re-find."
  {:added "1.0"
   :static true}
  [re s]
  (clojure.regex/matcher re s))
...
(defn re-groups
  ...
  {:added "1.0"
   :static true}
  [m]
  (let [gc (clojure.regex/-group-count m)]
    (if (zero? gc)
      (clojure.regex/-group m 0)
      (loop [ret [] c 0]
        (if (<= c gc)
          (recur (conj ret (clojure.regex/-group m c)) (inc c))
          ret)))))

(defn re-seq
  "Returns a lazy sequence of successive matches of pattern in string,
  using find, each such match processed with re-groups."
  {:added "1.0"
   :static true}
  [re s]
  (let [m (re-matcher re s)]
    ((fn step []
       (when (clojure.regex/-find m)
         (cons (re-groups m) (lazy-seq (step))))))))

(defn re-matches
  "Returns the match, if any, of string to pattern, using
  matches. Uses re-groups to return the groups."
  {:added "1.0"
   :static true}
  [re s]
  (let [m (re-matcher re s)]
    (when (clojure.regex/-matches m)
      (re-groups m))))

(defn re-find
  "Returns the next regex match, if any, of string to pattern, using
  find. Uses re-groups to return the groups."
  {:added "1.0"
   :static true}
  ([m]
   (when (clojure.regex/-find m)
...
(defn file-seq
  ...
  [dir]
    (tree-seq
     clojure.io/directory?
     clojure.io/list-files
     (clojure.io/file dir)))
...
(defmacro with-precision
  ...
  or:    (with-precision 10 :rounding :half-down (/ 1M 3))

  The rounding mode is one of :ceiling, :floor, :half-up, :half-down,
  :half-even, :up, :down and :unnecessary; it defaults to :half-up."
  {:added "1.0"}
  [precision & exprs]
    (let [[body rm] (if (= (first exprs) :rounding)
                      [(next (next exprs)) (second exprs)]
                      [exprs :half-up])]
      `(binding [*math-context* (clojure.math/math-context ~precision ~rm)]
         ~@body)))
...
(defn empty
  ...
  {:added "1.0"
   :static true}
  [coll]
  (when (satisfies? p/ICollection coll)
    (.empty  coll)))
...
(defn float-array
  "Creates an array of floats"
  {
   :inline-arities #{1 2}
   :added "1.0"}
  ([size-or-seq] (h/-float-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-float-array (h/host) size init-val-or-seq)))
...
(defn boolean-array
  "Creates an array of booleans"
  {
   :inline-arities #{1 2}
   :added "1.1"}
  ([size-or-seq] (h/-boolean-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-boolean-array (h/host) size init-val-or-seq)))
...
(defn byte-array
  "Creates an array of bytes"
  {
   :inline-arities #{1 2}
   :added "1.1"}
  ([size-or-seq] (h/-byte-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-byte-array (h/host) size init-val-or-seq)))
...
(defn char-array
  "Creates an array of chars"
  {
   :inline-arities #{1 2}
   :added "1.1"}
  ([size-or-seq] (h/-char-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-char-array (h/host) size init-val-or-seq)))
...
(defn short-array
  "Creates an array of shorts"
  {
   :inline-arities #{1 2}
   :added "1.1"}
  ([size-or-seq] (h/-short-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-short-array (h/host) size init-val-or-seq)))
...
(defn double-array
  "Creates an array of doubles"
  {
   :inline-arities #{1 2}
   :added "1.0"}
  ([size-or-seq] (h/-double-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-double-array (h/host) size init-val-or-seq)))
...
(defn object-array
  "Creates an array of objects"
  {
   :inline-arities #{1}
   :added "1.2"}
  ([size-or-seq] (h/-array (h/host) size-or-seq)))
...
(defn int-array
  "Creates an array of ints"
  {
   :inline-arities #{1 2}
   :added "1.0"}
  ([size-or-seq] (h/-int-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-int-array (h/host) size init-val-or-seq)))
...
(defn long-array
  "Creates an array of longs"
  {
   :inline-arities #{1 2}
   :added "1.0"}
  ([size-or-seq] (h/-long-array (h/host) size-or-seq))
  ([size init-val-or-seq] (h/-long-array (h/host) size init-val-or-seq)))
...
(defn seque
  ...
  size, or an instance of clojure.concurrent IBlockingQueue. Note
  ...
  ([s] (seque 100 s))
  ([n-or-q s]
   (let [q (if (clojure.concurrent/blocking-queue? n-or-q)
             n-or-q
             (clojure.concurrent/linked-blocking-queue (int n-or-q)))
         NIL (Object.) ;nil sentinel since LBQ doesn't support nils
         agt (agent (lazy-seq s)) ; never start with nil; that signifies we've already put eos
         log-error (fn [q e]
                     (if (clojure.concurrent/-offer q q)
                       (throw e)
                       e))
         fill (fn [s]
                (when s
                  (if (instance? Exception s) ; we failed to offer an error earlier
                    ...
                    (try
                      (loop [[x & xs :as s] (seq s)]
                        (if s
                          (if (clojure.concurrent/-offer q (if (nil? x) NIL x))
                            (recur xs)
                            s)
                          (when-not (clojure.concurrent/-offer q q) ; q itself is eos sentinel
                            ()))) ; empty seq, not nil, so we know to put eos next time
                      (catch Exception e
                        (log-error q e))))))
         drain (fn drain []
                 (lazy-seq
                  (let [x (clojure.concurrent/-take-blocking q)]
...
(defn- is-annotation? [c]
  (and (class? c)
       (.isAssignableFrom t/Annotation c)))

(defn- is-runtime-annotation? [ c]
  (boolean 
   (and (is-annotation? c)
        (when-let [.Retention r 
                   (t/get-annotation c t/Retention)] 
          (= (t/annotation-value r) t/RetentionPolicy/RUNTIME)))))
  ...
  (cond
   ...
   (symbol? v) (let [ev (eval v)]
                 (cond 
                  ...
                  :else (throw (ex-info "Unsupported annotation value: " v " of class " (class ev {})))))
...
(defn descendants
  ...
  ([h tag] (if (class? tag)
             (throw (ex-info "Can't get descendants of classes" {:tag tag :type :unsupported-operation}))
...
(defn iterator-seq
  ...
  {:added "1.0"
   :static true}
  [iter]
  (t/chunk-iterator-seq iter))

(defn enumeration-seq
  "Returns a seq on an enumeration"
  {:added "1.0"
   :static true}
  [e]
...
(defn format
  "Formats a string using printf-style format syntax"
  {:added "1.0"
   :static true}
   [fmt & args]
...
(defmacro with-loading-context [& body]
  `((fn loading# [] 
        (. clojure.lang.Var (pushThreadBindings {clojure.lang.Compiler/LOADER  
                                                 (.getClassLoader (.getClass  loading#))}))
        (try
         ~@body
         (finally
          (. clojure.lang.Var (popThreadBindings)))))))
  ...
  (if (qualified-symbol? sym)
    (or (resolve sym)
        (do (-> sym namespace symbol serialized-require)
            (resolve sym)))
    (throw (ex-info "Not a qualified symbol: " sym {}))))
    ...
    (let [ path (if (.startsWith path "/")
      ...
      (when-not (= path (first *pending-paths*))
        (binding [*pending-paths* (conj *pending-paths* path)]
          (t/load-resource (.substring path 1)))))))
  ...
  (let [gpred (gensym "pred__")
        ...
        emit (fn emit [pred expr args]
               (let [[[a b c :as clause] more]
                       ...
                       n (count clause)]
                 (cond
                  (= 0 n) `(throw (ex-info "No matching clause: " ~expr {}))
                  (= 1 n) a
                  (= 2 n) `(if (~pred ~a ~expr)
                             ~b
                             ~(emit pred expr more))
...
(defn future?
  ...
  {:added "1.1"
   :static true}
  [x] (clojure.concurrent/future? x))

(defn future-done?
  "Returns true if future f is done"
  {:added "1.1"
   :static true}
  [f] (clojure.concurrent/-done? f))
...
(defn- merge-hash-collisions
  ...
  (let [buckets (loop [m {} ks tests vs thens]
                  (if (and ks vs)
                    (recur
                      (update m (p/-hash (first ks)) (fnil conj []) [(first ks) (first vs)])
                      (next ks) (next vs))
                    m))
...
(defmacro case 
  ...
  (let [ge (with-meta (gensym) {:tag Object})
        default (if (odd? (count clauses)) 
                  (last clauses)
                  `(throw (ex-info "No matching clause: " ~ge {})))]
    (if (> 2 (count clauses))
      `(let [~ge ~e] ~default)
      (let [pairs (partition 2 clauses)
            assoc-test (fn assoc-test [m test expr]
                         (if (contains? m test)
                           (throw (ex-info "Duplicate case test constant: " test {}))
                           (assoc m test expr)))
            pairs (reduce1
                       (fn [m [test expr]]
                         (if (seq? test)
...
(defn stream-reduce!
  "Works like reduce but takes a stream as its source.
  Honors 'reduced', is a terminal operation on the stream"
  {:added "1.12"}
  ([f s]
   (clojure.core.protocols/iterator-reduce! (clojure.stream/-stream-iterator s) f))
  ([f init s]
   (clojure.core.protocols/iterator-reduce! (clojure.stream/-stream-iterator s) f init)))

(defn stream-seq!
  "Takes a stream and returns a seq of its contents.
  This is a terminal operation on the stream."
  {:added "1.12"}
  [stream]
  (iterator-seq (clojure.stream/-stream-iterator stream)))

(defn stream-transduce!
  "Works like transduce but takes a stream as its source.
  This is a terminal operation on the stream."
  {:added "1.12"}
...
(defn inst-ms
  ...
  {:added "1.9"}
  [inst]
  (clojure.time/-to-epoch-ms inst))

(defn inst?
  "Return true if x satisfies IInstant"
  {:added "1.9"}
  [x]
  (satisfies? clojure.time/IInstant x))
...
(defn uuid?
  ...
  {:added "1.9"}
  [x] (clojure.uuid/uuid? x))

(defn random-uuid
  "Returns a pseudo-randomly generated UUID (version 4)."
  {:added "1.11"}
  [] (clojure.uuid/random-uuid))
...
(extend-protocol clojure.core.protocols/IKVReduce
 ...
 (kv-reduce
  ...
  (reduce (fn [ret me]
            (f ret
               (clojure.lang.protocols/-key me)
               (clojure.lang.protocols/-val me)))
          init
          amap))
...
(defn slurp
  ...
  See clojure.io/reader for a complete list of supported arguments."
  {:added "1.0"
   :tag String}
  ([f & opts]
     (let [opts (normalize-slurp-opts opts)
           sw (clojure.io/string-writer)]
       (with-open [r (apply clojure.io/reader f opts)]
         (clojure.io/copy r sw)
         (clojure.io/string-writer-value sw)))))

(defn spit
  "Opposite of slurp.  Opens f with writer, writes content, then
  closes f. Options passed to clojure.io/writer."
  {:added "1.2"}
  [f content & options]
  (with-open [w (apply clojure.io/writer f options)]
    (.write w (str content))))
...
(defn future-call
  ...
  (let [f (binding-conveyor-fn f)
    ...
    (reify
     ...
     (isRealized [_] (clojure.concurrent/-done? fut))
     clojure.concurrent/IFuture
      (-deref-future [_] (clojure.concurrent/-deref-future fut))
      (-deref-future-timeout [_ timeout-ms timeout-val]
        (clojure.concurrent/-deref-future-timeout fut timeout-ms timeout-val))
      (-done? [_] (clojure.concurrent/-done? fut))
      (-cancel [_] (clojure.concurrent/-cancel fut))
      (-cancelled? [_] (clojure.concurrent/-cancelled? fut)))))
...
(defn future-cancel
  ...
  {:added "1.1"
   :static true}
  [f] (clojure.concurrent/-cancel f))

(defn future-cancelled?
  "Returns true if future f is cancelled"
  {:added "1.1"
   :static true}
  [f] (clojure.concurrent/-cancelled? f))
...
(let [properties (with-open [version-stream (.getResourceAsStream
                   ...
                   (clojure.io/load-properties version-stream))
      version-string (get properties "version")
      [_ major minor incremental qualifier snapshot]
      (re-matches
       #"(\d+)\.(\d+)\.(\d+)(?:-([a-zA-Z0-9_]+))?(?:-(SNAPSHOT))?"
       version-string)
      clojure-version {:major       (Integer/valueOf  major)
                       :minor       (Integer/valueOf  minor)
                       :incremental (Integer/valueOf  incremental)
                       :qualifier   (if (= qualifier "SNAPSHOT") nil qualifier)}]
  (def ^:dynamic *clojure-version*
    (if (.contains version-string "SNAPSHOT")
      (p/-assoc clojure-version :interim true)
      clojure-version)))
...
(defn promise
  ...
  {:added "1.1"
   :static true}
  []
  (let [d (clojure.concurrent/countdown-latch 1)
        v (atom d)]
    (reify
     clojure.lang.IDeref
       (deref [_] (clojure.concurrent/-await-latch d) @v)
     clojure.lang.IBlockingDeref
       (deref
        [_ timeout-ms timeout-val]
        (if (clojure.concurrent/-await-latch-timeout d timeout-ms)
          @v
          timeout-val))
     clojure.lang.IPending
      (isRealized [this]
       (zero? (clojure.concurrent/-latch-count d)))
     p/IFn
     (invoke
      [this x]
      (when (and (pos? (clojure.concurrent/-latch-count d))
                 (compare-and-set! v d x))
        (clojure.concurrent/-countdown d)
        this)))))
...
(defn partition-by
  ...
  ([f]
  (fn [rf]
    (let [a (clojure.host/make-buffer)
          pv (volatile! ::none)]
      (fn
        ([] (rf))
        ([result]
           (let [result (if (clojure.host/buf-empty? a)
                          result
                          (let [v (vec (clojure.host/buf-to-array a))]
                            ;;clear first!
                            (clojure.host/buf-clear a)
...
(defn partition-all
  ...
  ([^long n]
   (fn [rf]
     (let [a (clojure.host/make-buffer n)]
       (fn
         ([] (rf))
         ([result]
            (let [result (if (clojure.host/buf-empty? a)
...
(defn- ^{:dynamic true} assert-valid-fdecl
  ...
  (let [argdecls (map 
                   #(if (seq? %)
                      (first %)
                      (throw (ex-info (if (seq? (first fdecl {}))
...
(defn- load-data-reader-file [mappings ^java.net.URL url]
  (with-open [rdr (clojure.lang.LineNumberingPushbackReader.
                   (clojure.io/input-stream-reader
...
(defonce ^:private tapq (clojure.concurrent/array-blocking-queue 1024))

(defonce ^:private tap-loop
  (delay
   (doto (Thread.
          #(let [t (clojure.concurrent/-take-blocking tapq)
...
(defn tap>
  ...
  [x]
  (force tap-loop)
  (clojure.concurrent/-offer tapq (if (nil? x) ::tap-nil x)))
  ...
  (with-meta
    (persistent!
     (reduce-kv (fn [acc k v] (assoc! acc k (f v)))
                (if (satisfies? p/IEditableCollection m)
                  (transient m)
                  (transient {}))
                m))
    (meta m)))
