...
// ============================================================================
// Runtime Views — trace events → bitmasks (same query primitives as static views)
//
// tap_to_bitmask(): scan ring buffer, build u64* bitmask from arg0 of matching events
// tap_hit_counts(): count hits per arg0 slot
// These turn trace events into the same bitmask format as grammar.c views.
// ============================================================================

// Build bitmask from trace: for events of given kind, set bit arg0 in m
// m must be pre-allocated and zeroed (mw u64 words)
static void tap_to_bitmask(u32 kind, u64 *m, u32 mw) {
    ...
    for (u32 i = 0; i < n; i++) {
        u32 idx = (g_trace_pos - 1 - i) & TRACE_MASK;
        TraceEv *e = &g_trace[idx];
        if (e->kind == kind && e->arg0 < maxbit)
            m[e->arg0 / 64] |= (1ULL << (e->arg0 % 64));
    }
}

// Count hits per arg0 slot for events of given kind
// counts must be pre-allocated and zeroed (cap entries)
static void tap_hit_counts(u32 kind, u32 *counts, u32 cap) {
    u32 n = trace_count();
    for (u32 i = 0; i < n; i++) {
        u32 idx = (g_trace_pos - 1 - i) & TRACE_MASK;
        TraceEv *e = &g_trace[idx];
        if (e->kind == kind && e->arg0 < cap)
            counts[e->arg0]++;
    }
}
