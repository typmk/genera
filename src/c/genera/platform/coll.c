/**
 * coll.c — Persistent Data Structures: pmap (HAMT), pvec (32-way trie), atom
 *
 * Ported from proto_coll.c to freestanding. Uses base.c arena system.
 * Design: Clojure's PersistentHashMap/PersistentVector.
...
typedef struct { u32 bitmap; } CNode;
...
static CPMap cpmap_empty(void) {
    return (CPMap){0, COLL_NIL_IDX, true};
}
    ...
    if (m.is_small) {
        ...
        for (u32 i = 0; i < old->count; i++) {
            if (old->keys[i] == key) {
                ...
                return (CPMap){m.count, new_sm, true};
            }
        }
        if (old->count < SMALL_MAP_MAX) {
            u32 new_sm = csmall_put(m.root, key, val, &added);
            return (CPMap){m.count + 1, new_sm, true};
        }
        // Promote to HAMT
        ...
        return (CPMap){m.count + (added ? 1 : 0), root, false};
    }
    u32 new_root = chamt_put(m.root, 0, hash32(key), key, val, &added);
    return (CPMap){m.count + (added ? 1 : 0), new_root, false};
}
...
// ============================================================================
// 7. Protocol Implementations for PMAP/PVEC
// ============================================================================
...
static Val _pmap_seq(Val m) {
    return ((CPMap *)val_as_pmap(m))->count > 0 ? m : NIL;
}
