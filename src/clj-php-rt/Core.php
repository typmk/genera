...
use Clojure\Lang\ArraySeq;
use Clojure\Lang\Atom;
use Clojure\Lang\Cons;
use Clojure\Lang\ISeq;
use Clojure\Lang\Keyword;
use Clojure\Lang\LazySeq;
...
final class Core
{
    ...
    {
        return $x instanceof ISeq || is_array($x) || is_iterable($x);
    }
    ...
    public static function empty_QMARK_(mixed $x): bool
    {
        ...
        if ($x instanceof ISeq) return $x->seq() === null;
     ...
     * Handles ISeq, arrays, strings, and iterables.
     *
     * @return ISeq|array|null
     */
    public static function seq(mixed $x): ISeq|array|null
    {
        if ($x === null) return null;
        if ($x instanceof ISeq) return $x->seq();
    ...
    {
        if ($x === null) return null;
        if ($x instanceof ISeq) return $x->first();
     ...
     * Unlike next(), never returns nil.
     *
     * @return ISeq|array
     */
    public static function rest(mixed $x): ISeq|array
    {
        if ($x === null) return [];
        if ($x instanceof ISeq) return $x->more();
    ...
    /**
     * Returns a seq of the items after the first, or nil if there are no more.
     * Unlike rest(), returns nil for empty.
     *
     * @return ISeq|array|null
     */
    public static function next(mixed $x): ISeq|array|null
    {
        if ($x === null) return null;
        if ($x instanceof ISeq) return $x->next();
        $r = self::rest($x);
        if ($r instanceof ISeq) return $r->seq();
     ...
     * Returns a Cons cell for lazy seq compatibility.
     */
    public static function cons(mixed $x, mixed $seq): Cons
    {
        return new Cons($x, $seq);
    }

    /**
     * Creates a lazy sequence from a thunk (zero-arg function).
     * The thunk is only called when the seq is first accessed.
     */
    public static function lazy_seq(callable $thunk): LazySeq
    {
        return new LazySeq($thunk);
    }

    /**
     * Check if a lazy sequence has been realized.
     */
    public static function realized_QMARK_(mixed $x): bool
    {
        if ($x instanceof LazySeq) {
            return $x->isRealized();
        }
        // Non-lazy things are always "realized"
        return true;
    }

    /**
     * Force realization of a lazy seq if needed, returns the value.
     */
    public static function doall(mixed $coll): mixed
    {
        // Force full realization by walking the seq
        $seq = self::seq($coll);
        while ($seq !== null) {
            if ($seq instanceof ISeq) {
                $seq = $seq->next();
            } else {
                break;
            }
    ...
    /**
     * Like doall, but walks n elements of a seq.
     */
    public static function dorun(mixed $n_or_coll, mixed $coll = null): mixed
    {
        if ($coll === null) {
            // (dorun coll) - realize entire seq, return nil
            self::doall($n_or_coll);
            return null;
        }
        // (dorun n coll) - realize n elements, return nil
        $n = $n_or_coll;
        $seq = self::seq($coll);
        while ($n > 0 && $seq !== null) {
            $n--;
    ...
    /**
     * Returns a lazy seq of the concatenation of the elements in the supplied colls.
     */
    public static function concat(mixed ...$colls): LazySeq
    {
        return new LazySeq(function () use ($colls) {
            // Skip empty colls at the front
            while (count($colls) > 0 && self::seq($colls[0]) === null) {
                array_shift($colls);
            }
            if (count($colls) === 0) return null;

            $seq = self::seq($colls[0]);
            if ($seq === null) return null;

            $first = $seq instanceof ISeq ? $seq->first() : $seq[0];
            $rest = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);

            // Remaining colls after exhausting first
            $restColls = array_slice($colls, 1);
            if ($rest !== null) {
                array_unshift($restColls, $rest);
            }

            return self::cons($first, self::concat(...$restColls));
        });
    }

    /**
     * Returns a lazy seq of xs, ys, etc. concatenated (alias for concat).
     */
    public static function lazy_cat(mixed ...$colls): LazySeq
    {
        return self::concat(...$colls);
    }

    /**
     * Returns a lazy sequence of x, (f x), (f (f x)) etc.
     */
    public static function iterate(callable $f, mixed $x): LazySeq
    {
        return new LazySeq(function () use ($f, $x) {
            return self::cons($x, self::iterate($f, $f($x)));
        });
    }

    /**
     * Returns a lazy (infinite or length n) sequence of xs.
     */
    public static function repeat(mixed $x, ?int $n = null): LazySeq
    {
        if ($n !== null && $n <= 0) {
            return new LazySeq(fn() => null);
        }
        return new LazySeq(function () use ($x, $n) {
            if ($n === null) {
                // Infinite
                return self::cons($x, self::repeat($x));
            }
            return self::cons($x, self::repeat($x, $n - 1));
        });
    }

    /**
     * Returns a lazy seq of numbers from start (inclusive) to end (exclusive), by step.
     */
    public static function range(?int $start = null, ?int $end = null, int $step = 1): LazySeq
    {
        // (range) => infinite from 0
        if ($start === null) {
            return self::iterate(fn($x) => $x + 1, 0);
        }
        // (range end) => 0 to end
        if ($end === null) {
            $end = $start;
        ...
        return new LazySeq(function () use ($start, $end, $step) {
            if ($step > 0 && $start >= $end) return null;
            if ($step < 0 && $start <= $end) return null;
            return self::cons($start, self::range($start + $step, $end, $step));
        });
    }
     ...
     */
    public static function interleave(mixed ...$colls): LazySeq
    {
        return new LazySeq(function () use ($colls) {
            $seqs = array_map(fn($c) => self::seq($c), $colls);
            // If any is empty, stop
            foreach ($seqs as $s) {
                if ($s === null) return null;
            }
            // Get all firsts
            $firsts = array_map(fn($s) => $s instanceof ISeq ? $s->first() : $s[0], $seqs);
            // Get all rests
            $rests = array_map(function ($s) {
                if ($s instanceof ISeq) return $s->next();
                return count($s) > 1 ? array_slice($s, 1) : null;
            }, $seqs);
            // Interleave firsts, then recurse on rests
            return self::concat($firsts, self::interleave(...$rests));
        });
    }

    /**
     * Returns a lazy seq of the items in coll with sep between each.
     */
    public static function interpose(mixed $sep, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($sep, $coll) {
            $seq = self::seq($coll);
            if ($seq === null) return null;
            $first = $seq instanceof ISeq ? $seq->first() : $seq[0];
            $rest = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);
            if (self::seq($rest) === null) {
                return self::cons($first, null);
            }
            return self::cons($first, self::cons($sep, self::interpose($sep, $rest)));
        });
    }

    /**
     * Returns a lazy sequence of applying concat to the result of applying map to f and colls.
     */
    public static function mapcat(callable $f, mixed ...$colls): LazySeq
    {
        return self::concat(...self::map($f, ...$colls)->toArray());
    }

    /**
     * Returns a lazy sequence consisting of coll without duplicates.
     */
    public static function distinct(mixed $coll): LazySeq
    {
        $seen = [];
        $step = null;
        $step = function ($xs) use (&$step, &$seen) {
            return new LazySeq(function () use ($xs, &$step, &$seen) {
                $seq = Core::seq($xs);
                while ($seq !== null) {
                    $first = $seq instanceof ISeq ? $seq->first() : $seq[0];
                    $rest = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);
                    // Use spl_object_hash for objects, serialize for others
                    $key = is_object($first) ? spl_object_hash($first) : serialize($first);
                    if (!isset($seen[$key])) {
                        $seen[$key] = true;
                        return Core::cons($first, $step($rest));
                    }
                    $seq = Core::seq($rest);
                }
                return null;
    ...
    {
        if ($x === null) return 0;
        if ($x instanceof ISeq) return $x->count();
    ...
    public static function nth(mixed $coll, int $n, mixed $notFound = null): mixed
    {
        ...
        if ($coll instanceof ISeq) {
            $seq = $coll;
            for ($i = 0; $i < $n && $seq !== null; $i++) {
                $seq = $seq->next();
            }
            return $seq !== null ? $seq->first() : $notFound;
        }
        if (is_array($coll)) {
     ...
     */
    public static function take(int $n, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($n, $coll) {
            ...
            if ($seq === null) return null;
            if ($seq instanceof ISeq) {
                return self::cons($seq->first(), self::take($n - 1, $seq->next()));
            }
            // Array case
            return self::cons($seq[0] ?? null, self::take($n - 1, array_slice($seq, 1)));
        });
    }
     ...
     */
    public static function drop(int $n, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($n, $coll) {
            $seq = self::seq($coll);
            for ($i = 0; $i < $n && $seq !== null; $i++) {
                if ($seq instanceof ISeq) {
                    ...
                    $seq = count($seq) > 1 ? array_slice($seq, 1) : null;
                } else {
                    $seq = null;
                }
            }
    ...
    /**
     * Returns a lazy sequence of items in coll while pred returns true.
     */
    public static function take_while(callable $pred, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($pred, $coll) {
            ...
            $rest = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);
            return self::cons($first, self::take_while($pred, $rest));
        });
    }

    /**
     * Returns a lazy sequence of items after pred returns false.
     */
    public static function drop_while(callable $pred, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($pred, $coll) {
            $seq = self::seq($coll);
            while ($seq !== null) {
                $first = $seq instanceof ISeq ? $seq->first() : $seq[0];
                if (!$pred($first)) return $seq;
                $seq = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);
            }
            return null;
        });
    }
    ...
    /**
     * Returns a lazy sequence of applying f to each item in coll.
     * For multiple colls, applies f to corresponding items until shortest exhausted.
     */
    public static function map(callable $f, mixed ...$colls): LazySeq
    {
        if (count($colls) === 0) {
            return new LazySeq(fn() => null);
        }
        ...
        if (count($colls) === 1) {
            ...
            return new LazySeq(function () use ($f, $coll) {
                ...
                if ($seq === null) return null;
                if ($seq instanceof ISeq) {
                    return self::cons($f($seq->first()), self::map($f, $seq->next()));
                }
                // Array case
                return self::cons($f($seq[0]), self::map($f, count($seq) > 1 ? array_slice($seq, 1) : null));
            });
        }

        // Multi-collection map
        return new LazySeq(function () use ($f, $colls) {
            $seqs = array_map(fn($c) => self::seq($c), $colls);
            // If any seq is null, we're done
            foreach ($seqs as $s) {
                if ($s === null) return null;
            }
            // Get first from each
            ...
            return self::cons($f(...$firsts), self::map($f, ...$rests));
        });
    }

    /**
     * Returns a vector from applying f to each item in coll(s).
     * Eager version of map.
     */
    public static function mapv(callable $f, mixed ...$colls): array
    {
        $result = self::map($f, ...$colls);
        return $result instanceof ISeq ? $result->toArray() : (array) $result;
    }

    /**
     * Returns a lazy sequence of items in coll for which pred returns true.
     */
    public static function filter(callable $pred, mixed $coll): LazySeq
    {
        return new LazySeq(function () use ($pred, $coll) {
            $seq = self::seq($coll);
            while ($seq !== null) {
                ...
                $rest = $seq instanceof ISeq ? $seq->next() : (count($seq) > 1 ? array_slice($seq, 1) : null);
                if ($pred($first)) {
                    return self::cons($first, self::filter($pred, $rest));
                }
                $seq = self::seq($rest);
            }
            return null;
    ...
    /**
     * Returns a vector of items in coll for which pred returns true.
     * Eager version of filter.
     */
    public static function filterv(callable $pred, mixed $coll): array
    {
        $result = self::filter($pred, $coll);
        return $result instanceof ISeq ? $result->toArray() : [];
    }

    /**
     * Returns a lazy sequence of items in coll for which pred returns false.
     */
    public static function remove(callable $pred, mixed $coll): LazySeq
    {
    ...
    public static function reduce(callable $f, mixed ...$args): mixed
    {
        if (count($args) === 1) {
            ...
            // Array case
            return array_reduce(array_values($seq), $f, $init);
        }
    }
